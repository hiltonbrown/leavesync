...
Authorised users can always view and copy the complete active feed URL. Team Calendar never masks, truncates, or replaces the subscribe URL with a hash. Pausing a feed keeps the URL available, while rotating or revoking its token invalidates the old URL.

Internal signing and verification material remains server-only and is never shown in place of the usable URL. A feed's **Masked** privacy setting affects event details, not the feed URL.
